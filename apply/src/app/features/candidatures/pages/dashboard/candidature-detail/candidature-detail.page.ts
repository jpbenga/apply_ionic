...
import { getFrenchCoverLetterHtml } from 'src/app/services/pdf-generator/letter.template';
...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, RouterModule, FormsModule, IonHeader, IonContent, IonToolbar
  ]
})
export class CandidatureDetailPage implements OnInit, OnDestroy {
  ...
  async downloadLettreMotivation() {
    if (!this.candidature || !this.candidature.lettreMotivationGeneree) {
      this.presentToast('Aucune lettre de motivation générée disponible.', 'warning');
      return;
    }
    ...
    let contentToRender: HTMLElement | null = null;
    ...
    try {
      ...
      const letterHtml = getFrenchCoverLetterHtml(
        userProfile,
        this.candidature,
        this.candidature.lettreMotivationGeneree
      );

      const letterElement = document.createElement('div');
      letterElement.innerHTML = letterHtml;
      contentToRender = letterElement.firstElementChild as HTMLElement;

      if (!contentToRender) {
        throw new Error("Le template de la lettre n'a pas pu être créé.");
      }

      document.body.appendChild(contentToRender);
      await new Promise(resolve => setTimeout(resolve, 50));

      const validation = this.pdfGeneratorService.validateElement(contentToRender);
      if (!validation.valid) {
        throw new Error(`Élément HTML invalide pour le PDF: ${validation.errors.join(', ')}`);
      }
      ...
      await this.pdfGeneratorService.generateOptimizedPdf(
        contentToRender,
        { filename, singlePage: true, isPreStyled: true }
      );
      ...
      if (contentToRender && document.body.contains(contentToRender)) {
  ...
  getStatutColor(statut: StatutCandidature): string {
    const colorMap: { [key in StatutCandidature]: string } = {
      ...
      'entretien_planifie': 'warning', 'test_technique': 'warning', 'entretien_final': 'warning',
      'offre_recue': 'success', 'acceptee': 'success', 'refusee_candidat': 'danger',
      'refusee_entreprise': 'danger', 'archivee': 'medium', 'standby': 'medium'
    };
  ...
  getSuiviIcon(type: TypeSuivi): string {
    const iconMap: { [key in TypeSuivi]: string } = {
      'contact': '👤', 'email_envoye': '📧', 'email_recu': '📬',
      'appel_effectue': '📞', 'appel_recu': '📞', 'entretien_planifie': '📅',
      'entretien_effectue': '✅', 'entretien': '👥', 'test_technique_recu': '📝',
      'test_technique_complete': '📋', 'test': '🎯', 'feedback': '💬',
      'relance': '🔄', 'autre': '📌'
    };
    return iconMap[type] || '📌';
  }
  ...
  getSuiviColor(type: TypeSuivi): string {
    const colorMap: { [key in TypeSuivi]: string } = {
      ...
      'appel_effectue': 'warning', 'appel_recu': 'warning', 'entretien_planifie': 'warning',
