...
export class PdfGeneratorService {
  ...
  async generateOptimizedPdf(
    ...
    try {
      console.log('🔄 Début génération PDF optimisée via PdfGeneratorService');
      ...
      try {
        ...
        console.log('📊 Métriques du layout:', metrics);
        ...
        console.log('✅ PDF généré avec succès:', config.filename);
  ...
  private applyContentCompression(element: HTMLElement, metrics: PdfLayoutMetrics): void {
    ...
    console.log(`🗜️ Application compression: ${Math.round(metrics.compressionNeeded * 100)}%`);
